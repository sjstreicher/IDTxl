...
class NetworkInference(NetworkAnalysis):
    ...
    def _initialise(self, options, data, sources, target):
        ...
        if type(options['min_lag_sources']) is not int or options['min_lag_sources'] < 0:
            raise RuntimeError('min_lag_sources has to be an integer >= 0.')
        if type(options['max_lag_sources']) is not int or options['max_lag_sources'] < 0:
            raise RuntimeError('max_lag_sources has to be an integer >= 0.')
        if type(options['max_lag_target']) is not int or options['max_lag_target'] <= 0:
            raise RuntimeError('max_lag_target must be an integer > 0.')
        if type(options['tau_sources']) is not int or options['tau_sources'] <= 0:
            raise RuntimeError('tau_sources must be an integer > 0.')
        if type(options['tau_target']) is not int or options['tau_target'] <= 0:
            raise RuntimeError('tau_sources must be an integer > 0.')
