...
class OpenCLKraskov(Estimator):
    ...
    def __init__(self, settings=None):
        ...
        if self.settings["return_counts"] and not self.settings["debug"]:
            raise RuntimeError("Set debug option to True to return neighbor counts.")
...
class OpenCLKraskovMI(OpenCLKraskov):
    ...
    def estimate(self, var1, var2, n_chunks=1):
        ...
        logger.debug(
            ...
            "per run: {2}.".format(mem_chunk / C, max_mem / C, chunks_per_run)
        )
    ...
    def _estimate_single_run(self, var1, var2, n_chunks=1):
        ...
        logger.debug("NDRange_x: {}, workitems_x: {}".format(NDRange_x, workitems_x))
        ...
        if self.settings["local_values"]:
            ...
            for c in range(n_chunks):
                ...
                mi_array[c] = mi
        assert (
            signallength_orig == (c + 1) * chunklength
        ), "Original signal length does not match no. processed points."
