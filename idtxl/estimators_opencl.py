...
try:
    ...
    ex.package_missing(
        err,
        "PyOpenCl is not available on this system. Install"
...
class OpenCLKraskov(Estimator):
    ...
    def __init__(self, settings=None):
        ...
        self.kernel_location = resource_filename(__name__, "gpuKnnKernelNoIdx.cl")
    ...
    def _get_kernels(self):
        ...
        kNN_kernel.set_scalar_arg_dtypes(
            [None, None, None, np.int32, np.int32, np.int32, np.int32, np.int32, None]
        )

        RS_kernel = program.kernelBFRSAllshared
        RS_kernel.set_scalar_arg_dtypes(
    ...
    def _get_max_mem(self):
        ...
        if "max_mem" in self.settings:
            ...
            return (
                self.settings["max_mem_frac"]
                * self.devices[self.settings["gpuid"]].global_mem_size
            )
        else:
...
class OpenCLKraskovMI(OpenCLKraskov):
    ...
    def estimate(self, var1, var2, n_chunks=1):
        ...
        if self.settings["debug"]:
            print(
                ...
                "chunks per run: {2}.".format(
                    mem_chunk / 1024 / 1024, max_mem / 1024 / 1024, chunks_per_run
                )
            )
    ...
    def _estimate_single_run(self, var1, var2, n_chunks=1):
        ...
        if self.settings["debug"]:
            print(
                "var1 shape: {0}, {1}, n_chunks: {2}".format(
        ...
        pad_target = 1024
        pad_size = int(np.ceil(signallength / pad_target)) * pad_target - signallength
        pad_var1 = np.vstack([var1, 999999 + 0.1 * np.random.rand(pad_size, var1dim)])
        pad_var2 = np.vstack([var2, 999999 + 0.1 * np.random.rand(pad_size, var2dim)])
        ...
        if self.settings["debug"]:
            ...
            mem_data_pad = self.sizeof_float * pointset.shape[0] * pointset.shape[1]
            mem_dist = (
                self.sizeof_float * signallength_padded * self.settings["kraskov_k"]
            )
            mem_ncnt = 2 * self.sizeof_int * signallength_padded
            mem_total = mem_data_pad + mem_dist + mem_ncnt
            print(
                "Memory req. after padding: {0:.5f} MB.".format(mem_total / 1024 / 1024)
            )

        # Set OpenCL kernel launch parameters
        if chunklength < self.devices[self.settings["gpuid"]].max_work_group_size:
            workitems_x = 8
        elif self.devices[self.settings["gpuid"]].max_work_group_size < 256:
            workitems_x = self.devices[self.settings["gpuid"]].max_work_group_size
        else:
            workitems_x = 256
        NDRange_x = workitems_x * (int((signallength_padded - 1) / workitems_x) + 1)
        ...
        d_var1 = d_pointset.get_sub_region(
            0, self.sizeof_float * signallength_padded * var1dim, cl.mem_flags.READ_ONLY
        )
        ...
        d_npointsrange_x = cl.Buffer(
            self.context, cl.mem_flags.READ_WRITE, self.sizeof_int * signallength_padded
        )
        d_npointsrange_y = cl.Buffer(
            self.context, cl.mem_flags.READ_WRITE, self.sizeof_int * signallength_padded
        )
        ...
        if self.settings["local_values"]:
            mi_array = -np.inf * np.ones(chunklength * n_chunks, dtype=np.float64)
            idx = 0
            ...
            for c in range(n_chunks):
                mi = (
                    ...
                    - np.mean(
                        digamma(count_var1[c * chunklength : (c + 1) * chunklength] + 1)
                        + digamma(
                            count_var2[c * chunklength : (c + 1) * chunklength] + 1
                        )
                    )
                )
                mi_array[c] = mi
...
class OpenCLKraskovCMI(OpenCLKraskov):
    ...
    def estimate(self, var1, var2, conditional=None, n_chunks=1):
        ...
        for r in range(0, n_chunks, chunks_per_run):
            ...
            n_chunks_current_run = subset1.shape[0] // chunklength
            results = self._estimate_single_run(
                subset1, subset2, subset3, n_chunks_current_run
            )
            if self.settings["debug"]:
    ...
    def _estimate_single_run(self, var1, var2, conditional=None, n_chunks=1):
        ...
        # Calculate and sum digammas
        if self.settings["local_values"]:
            cmi_array = -np.inf * np.ones(n_chunks * chunklength, dtype=np.float64)
            idx = 0
            for c in range(n_chunks):
                cmi = (
                    digamma(kraskov_k)
