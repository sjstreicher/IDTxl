...
def test_knn_one_dim():
    ...
    (indexes2, distances2) = knn_search(
...
def test_knn_two_dim():
    ...
    (indexes2, distances2) = knn_search(
        pointset2, pointset2, knn_k, theiler_t, n_chunks, gpu_id
    )
...
def test_one_dim_longer_sequence():
    ...
    knn_k = 1
    n_chunks = 1
    pointset = np.array([-1, -1.2, 1, 1.1, 10, 11, 10.5, -100, -50, 666]).astype(
        "float32"
    )
    gpu_id = 0
...
def test_random_data():
    ...
    assert err == 1, "There was an error during the GPU-call."
    assert np.all(indexes == indexes2), (
        "High- and low-level calls returned " "different indices."
    )
...
def test_two_chunks():
    ...
    pointset = np.array([5, 6, -5, -7, 50, -50, 60, -70]).astype("float32")
...
def test_three_chunks():
    ...
    pointset = np.array([5, 6, -5, -7, 50, -50, 60, -70, 500, -500, 600, -700]).astype(
        "float32"
    )
