...
def clFindKnn(
    ...
    localmem = (
        numpy.dtype(numpy.float32).itemsize * kth * workitems_x
        + numpy.dtype(numpy.int32).itemsize * kth * workitems_x
    ) / 1024
    if localmem > my_gpu_devices[gpuid].local_mem_size / 1024:
        print(
            (
                "Localmem alocation will fail.",
                my_gpu_devices[gpuid].local_mem_size / 1024,
                "kb available, and it needs",
                localmem,
                "kb.",
            )
        )
    ...
    if (totalmem * 0.90) < usedmem:
        print(
            (
                "WARNING:",
                usedmem,
                "Mb used from a total of",
                totalmem,
                "Mb. GPU could get without memory",
            )
        )
