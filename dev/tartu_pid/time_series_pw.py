...
def test__solve_time_series(fun_obj, noise_level=.05, L_range=None, verbose=False):
    if L_range is None:
    ...
    for L in L_range:
        ...
        tic = tm.clock()
        retval = TartuSynergy.solve_PDF(
            pdf,
            fun_obj.true_input_distrib(),
        ...
        toc = tm.clock()
        print(
            "============================== Total elapsed time: {0} seconds".format(
                toc - tic
            )
        )
        ...
        print(
            ...
            feas,
            "  maximal KKT-system constraint violation:",
            kkt,
        )
        ...
        print(
            "\n\nError - CI: {0}, SI: {1}\n\n".format(
                CI - estimate["syn_s1_s2"], SI - estimate["shd_s1_s2"]
            )
        )
...
def test_speed(fun_obj, noise_level=.05, verbose=False, n_run=1):
    ...
    np.savetxt(
        "/home/patriciaw/repos/IDTxl/dev/tartu_pid/speedtest_results/"
        + fun_obj.name
        + "_ci_tartu_run_"
        + str(n_run)
        + ".csv",
