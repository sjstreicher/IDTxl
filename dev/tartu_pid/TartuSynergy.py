...
class Compute_UI:
    def __init__(self, marg_xy, marg_xz, _set_to_zero=set()):
        ...
        self.X = set(
            [x for x, y in self.orig_marg_xy.keys()]
    ...
    def create_equations(self):
        ...
        if rk != len(list_b):
            print(
                "BUG: There's something wrong with the rank of the coefficient matrix: it is ",
                rk,
                " it should be ",
                len(list_b),
            )
        ...
        for xyz, i in self.var_idx.items():
            ...
            for x_ in self.X:
                if x_ == x:  # diagonal
                    ...
                    columns.append(i)
                    tmp_quot = (
                        zz[0] * p_yz__x / p_yz[y, z]
                    )  # 1/p[x,y,z] - 1/p[*,y,z] = ( p[*,y,z] - p[x,y,z] )/( p[*,y,z] p[x,y,z] )
                    if p[i] > 0 * tmp_quot:
    ...
    def make_initial_solution(self, orig_q_0=None):
        ...
        if (
            orig_q_0 == None
        ):  # no initial solution provided. I'm gonna get some "minimum entropy" thing
        ...
        def __init__(self, cui):
            ...
            for x in self.cui.X:
                for y in self.cui.Y:
                    for z in self.cui.Z:
                        rhs = 0
                        self.constr_leq[x, y, z] = self.model.addConstr(
                            self.lambda_vars[x, y] + self.mu_vars[x, z] - rhs
                            <= self.t_var,
                            name="eq-le(%s,%s,%s)" % (x, y, z),
                        )
                        self.constr_geq[x, y, z] = self.model.addConstr(
                            self.lambda_vars[x, y] + self.mu_vars[x, z] - rhs
                            >= -self.t_var,
                            name="eq-ge(%s,%s,%s)" % (x, y, z),
                        )
                    # ^ for z
                # ^ for y
            # ^ for x
                        ...
                        for z in self.cui.Z:
                            ...
                            if xyz in p.keys() and p[xyz] > ZERO:
                                ...
                                if (y, z) in p_yz.keys() and p_yz[y, z] > 0:
                                    rhs = (
                                        1.e400
                                    )  # don't need ZERO here,  marginal_yz_w_cutoff()
                                else:
                                    rhs = 0.
                                self.constr_geq[xyz].setAttr("rhs", rhs)
                            # if
    ...
    def search_for_kkt_solution(
        self, p, stop_search_kkt_eps=-1., stop_search_prob_eps=1., show_progress=True
    ):
        min_viol = 1.e1000
        arg_min_viol = None
        max_viol = -1.e1000
        ...
        if min_viol > stop_search_kkt_eps:
            if show_progress:
                print(
                    "Oh bummer, that's too much KKT violation. Trying fallback: the_ZERO=",
                    last_best_guess,
                )
...
def print_solution_stats(
    soliter, the_p, pdf, true_pdf, true_result, true_CI, true_SI, feas=None, kkt=None
):
    if true_pdf != None:
        print(
            "SOLUTION #" + str(soliter) + ": d_TV(timeseries, true): ",
            total_variation_distance(pdf, true_pdf),
        )
    if true_result != None:
        print(
            "SOLUTION #" + str(soliter) + ": d_TV(result, true):     ",
            total_variation_distance(true_result, the_p),
        )
    if true_pdf != None:
        print(
            "SOLUTION #" + str(soliter) + ": supp-dist(timeseries, true): ",
            support_variation(pdf, true_pdf),
        )
    if true_result != None:
        print(
            "SOLUTION #" + str(soliter) + ": supp-dist(result, true):     ",
            support_variation(true_result, the_p),
        )
    if true_pdf != None:
        print(
            "SOLUTION #" + str(soliter) + ": KL-Divergence(true, timeseries): ",
            kl_divergence(true_pdf, pdf),
        )
    if true_result != None:
        print(
            "SOLUTION #" + str(soliter) + ": KL-Divergence(true, result):     ",
            kl_divergence(true_result, the_p),
        )
    print("SOLUTION #" + str(soliter) + ": CI-wriggle:", wriggle_CI(pdf, the_p))
    if true_CI != None:
        print(
            "SOLUTION #" + str(soliter) + ": CI-wriggle difference:",
            abs(wriggle_CI(pdf, the_p) - true_CI),
        )
    print("SOLUTION #" + str(soliter) + ": SI-wriggle:", wriggle_SI(the_p))
    if true_SI != None:
        print(
            "SOLUTION #" + str(soliter) + ": SI-wriggle difference:",
...
def solve_PDF(
    ...
    kkt_eps_2=.01,
    kkt_search_eps=.5,
    max_zero_probability=1.e-5,  # kkt_search_entropy_eps=1.e-5,
    verbose=False,
):
    if verbose:
        solvers.options[
            "show_progress"
        ] = True  # turns the output to the screen on or off
    else:
    ...
    for main_loop_iter in range(10):
        ...
        if true_result != None and verbose:
            print(
                "solve_PDF(): true_result:",
                sorted(true_result.items(), key=lambda i: i[0]),
            )
        the_p = cui.p_final
        ...
        if kkt <= kkt_eps and feas <= feas_eps:
            if verbose:
                print(
                    "solve_PDF(): Done. Returning a solution with 1-norm of primal violation",
                    feas,
                    "and maximum dual violation ",
                    kkt,
                )
            ...
            if verbose:
                print("solve_PDF(): Max ZERO probability = ", max_zero_probability)
            stop_search_kkt_eps = kkt_search_eps * kkt
            kkt_search = cui.search_for_kkt_solution(
                cui.p_final,
                show_progress=verbose,
                stop_search_kkt_eps=stop_search_kkt_eps,
                stop_search_prob_eps=max_zero_probability,
            )
            if kkt_search == None:
                if verbose:
                ...
                if feas <= feas_eps_2 and kkt <= kkt_eps_2:
                    ...
                    if verbose:
                        print(
                            "solve_PDF(): Returning a solution with 1-norm of primal violation",
                            feas,
                            "and maximum dual violation ",
                            kkt,
                        )
                ...
                if verbose:
                    print("solve_PDF(): Set everything leq", ZERO_min_viol, "to zero.")
                p_new = dict()
    ...
    print(
        "Giving up.  Returning a solution with 1-norm of primal violation",
        feas,
        "and maximum dual violation ",
        kkt,
    )
