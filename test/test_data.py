...
def test_data_properties():
    ...
    assert real_time == (n - cv[1]), (
        "Realisations in time are not "
...
def test_set_data():
    ...
    assert (data.data[0, :].T == source.T).all(), (
        "Class data does not match " "input (source)."
    )
    assert (data.data[1, :].T == target.T).all(), (
        "Class data does not match " "input (target)."
    )
    ...
    assert d.data.shape[0] == 2, (
        "Class data does not match input, number " "of processes wrong."
    )
    assert d.data.shape[1] == 1000, (
        "Class data does not match input, " "number of samples wrong."
    )
    assert d.data.shape[2] == 5, (
        "Class data does not match input, number " "of replications wrong."
    )
    ...
    data = np.arange(5000)
    d.set_data(data, "s")
    assert d.data.shape[0] == 1, (
        "Class data does not match input, number " "of processes wrong."
    )
    assert d.data.shape[1] == 5000, (
        "Class data does not match input, " "number of samples wrong."
    )
    assert d.data.shape[2] == 1, (
        "Class data does not match input, number " "of replications wrong."
...
def test_data_normalisation():
    ...
    assert (source_std == data.data[0, :, 0]).all(), (
        "Standardising the " "source did not work."
    )
    assert (target_std == data.data[1, :, 0]).all(), (
        "Standardising the " "target did not work."
    )
...
def test_permute_replications():
    ...
    n = 20
    data = Data(
        np.vstack((np.zeros(n), np.ones(n) * 1, np.ones(n) * 2, np.ones(n) * 3)).astype(
            int
        ),
        "rs",
        normalise=False,
    ...
    current_value = (0, n - 1)
    l = [(0, 1), (0, 3), (0, 7)]
    [perm, perm_idx] = data.permute_replications(
        current_value=current_value, idx_list=l
    )
    ...
    data = Data(
        np.vstack((np.arange(n), np.arange(n))).astype(int), "rs", normalise=False
    )
    perm_settings = {"perm_type": "local", "perm_range": rng}
    [perm, perm_idx] = data.permute_samples(
    ...
    for p in range(n_per_repl // rng):
        assert (
            np.unique(perm[i : i + rng, 0]) == samples
        ).all(), "The permutation range was not respected."
        samples += rng
        i += rng
    rem = n_per_repl % rng
    if rem > 0:
        assert (
            np.unique(perm[i : i + rem, 0]) == samples[0:rem]
        ).all(), "The remainder did not contain the same realisations."
...
def test_permute_samples():
    ...
    perm = data.permute_samples(
        current_value=(0, 0), idx_list=[(0, 0)], perm_settings=settings
    )[0]
    assert (
        sorted(np.squeeze(perm)) == np.arange(n)
    ).all(), "Permutation did not contain the correct values."
    ...
    assert (
        np.squeeze(np.vstack((perm[idx_start:], perm[:idx_start]))) == np.arange(n)
    ).all(), "Circular shifting went wrong."
...
def test_get_data_slice():
    ...
    for r in range(n_replications):
        assert s[0][r] == i[r], "Replication index {0} is not correct.".format(r)
...
def test_swap_blocks():
    ...
    assert sum(perm == n_blocks - 1) == 1, (
        "No. remaining samples in the last" " block is incorrect."
    )
...
def test_data_type():
    ...
    assert issubclass(type(data.data[0, 0, 0]), np.float), "Data type is not a float."
    ...
    assert issubclass(type(real[0, 0]), np.integer), "Realisations type is not an int."
    sl = data._get_data_slice(0)[0]
    assert issubclass(type(sl[0, 0]), np.integer), "Data slice type is not an int."
    settings = {"perm_type": "random"}
    sl_perm = data.slice_permute_samples(0, settings)[0]
    assert issubclass(
        type(sl_perm[0, 0]), np.integer
    ), "Permuted data slice type is not an int."
    samples = data.permute_samples((0, 5), [(1, 1), (1, 3)], settings)[0]
    assert issubclass(
        type(samples[0, 0]), np.integer
