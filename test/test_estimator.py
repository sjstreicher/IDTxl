...
def test_estimate_parallel():
    ...
    with pytest.raises(AssertionError):
        mi_estimator.estimate_parallel(n_chunks=2, var1=source_chunks, var2=target)
    mi_parallel1 = mi_estimator.estimate_parallel(
        n_chunks=2, re_use=["var2"], var1=source_chunks, var2=target
    )
    mi_parallel2 = mi_estimator.estimate_parallel(
        n_chunks=2, var1=source_chunks, var2=target_chunks
    )
    assert (
        mi_parallel1 == mi_parallel2
    ).all(), "Results for stacked ({0}) and re-used ({1}) target differ.".format(
        mi_parallel1, mi_parallel2
    )
    assert mi_parallel1[0] == mi, "Results for first chunk differ from serial estimate."
    assert (
        mi_parallel1[1] == mi
    ), "Results for second chunk differ from serial estimate."
    assert np.isclose(
        mi, expected_mi, rtol=0.05
    ), "Estimated ({0}) and expected ({1}) MI differ.".format(mi, expected_mi)

    # Check if a single chunk is returned if all variables are defined as
    # reusable.
    mi_parallel3 = mi_estimator.estimate_parallel(
        n_chunks=2, re_use=["var1", "var2"], var1=source1, var2=target
    )
    assert len(mi_parallel3) == 1, "Single chunk data returned more than one estimate."
    assert np.isclose(
        mi_parallel3[0], expected_mi, rtol=0.05
    ...
    with pytest.raises(AssertionError):
        mi_estimator.estimate_parallel(
            n_chunks=2, re_use=["var2"], var1=source_chunks, var2=target[:100]
        )
