...
def test_bbc_and_shuffling_estimator():
    ...
    est_bbc = RudeltBBCEstimator()
    I_bbc, R_bbc, bbc_term = est_bbc.estimate(
        symbol_array[0], past_symbol_array[0], current_symbol_array[0]
    )
    ...
    print(
        "BBC MI result: {0:.4f}; Shuffling MI result: {1:.4f}".format(
            I_bbc, I_shuffling
        )
    )
    print(
        "BBC MI / H_uncond result: {0:.4f}; Shuffling MI / H_uncond result: {1:.4f}".format(
            R_bbc, R_shuffling
        )
    )

    assert np.isclose(
        I_bbc, I_shuffling, atol=0.01
    ), "MI of BBC and Shuffling estimators are not close to each other (error larger 0.01)."

    assert np.isclose(
        R_bbc, R_shuffling, atol=0.01
