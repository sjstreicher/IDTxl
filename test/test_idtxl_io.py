...
def test_export_brain_net():
    ...
    adj_matrix = res_0.get_adjacency_matrix(weights="max_te_lag", fdr=False)
...
def test_import_matarray():
    ...
    ), "Wrong number of replications: {0}.".format(
        data.n_replications
    )
