...
optimiser_missing = pytest.mark.skipif(package_missing, reason="ECOS is missing.")
...
float128_not_available = pytest.mark.skipif(
    no_float128, reason="type float128 not available on current architecture"
)
...
def test_pip_source_copy():
    ...
    assert np.isclose(
        1, est_sydney["unq_s1"], atol=1.e-7
    ), "Unique information 1 is not 1 for Sydney estimator ({0}).".format(
        est_sydney["unq_s1"]
    )
    assert np.isclose(
        0, est_sydney["unq_s2"], atol=1.e-7
    ), "Unique information 2 is not 0 for Sydney estimator ({0}).".format(
        est_sydney["unq_s2"]
    )
    assert np.isclose(
        0, est_sydney["shd_s1_s2"], atol=1.e-7
    ), "Shared information is not 0 for Sydney estimator ({0}).".format(
        est_sydney["shd_s1_s2"]
    )
    assert np.isclose(
        0, est_sydney["syn_s1_s2"], atol=1.e-7
    ), "Synergy is not 0 for Sydney estimator ({0}).".format(est_sydney["syn_s1_s2"])
    assert np.isclose(
        0, est_tartu["shd_s1_s2"], atol=1.e-7
    ), "Shared information is not 0 for Tartu estimator ({0}).".format(
        est_tartu["shd_s1_s2"]
    )
    assert np.isclose(
        0, est_tartu["syn_s1_s2"], atol=1.e-7
    ), "Synergy is not 0 for Tartu estimator ({0}).".format(est_tartu["syn_s1_s2"])
...
def test_non_binary_alphabet():
    ...
    assert np.isclose(
        est_tartu["syn_s1_s2"], est_sydney["syn_s1_s2"], atol=1e-03
    ), "Synergies are not equal."
    assert np.isclose(
        est_tartu["shd_s1_s2"], est_sydney["shd_s1_s2"], atol=1e-03
    ), "Shareds are not equal."
    assert np.isclose(
        est_tartu["unq_s1"], est_sydney["unq_s1"], atol=1e-03
    ), "Unique1 is not equal."
    assert np.isclose(
        est_tartu["unq_s2"], est_sydney["unq_s2"], atol=1e-03
    ), "Unique2 is not equal."
