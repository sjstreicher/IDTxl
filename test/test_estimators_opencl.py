...
def test_amd_data_padding():
    ...
    for n in [832, 999, 10000, 3781, 50000]:
        ...
        print(
            "{0} points, {1} chunks: OpenCL MI result: {2:.4f} nats; "
        ...
        print(
            "OpenCL CMI result: {0:.4f} nats; expected to be close to "
            "{1:.4f} nats for correlated Gaussians.".format(cmi[0], expected_mi)
        )
...
def test_user_input():
    ...
    with pytest.raises(AssertionError):
        est_mi.estimate(var1=np.random.randn(N, 1), var2=np.random.randn(N + 1, 1))
    with pytest.raises(AssertionError):
        est_cmi.estimate(
            var1=np.random.randn(N, 1),
    ...
    with pytest.raises(AssertionError):
        est_mi.estimate(
            var1=np.random.randn(N, 1), var2=np.random.randn(N, 1), n_chunks=7
        )
    with pytest.raises(AssertionError):
        est_cmi.estimate(
            var1=np.random.randn(N, 1),
...
def test_mi_correlated_gaussians():
    ...
    print(
        "JIDT MI result: {0:.4f} nats; OpenCL MI result: {1:.4f} nats; "
...
def test_cmi_correlated_gaussians():
    ...
    settings = {"debug": True, "return_counts": True}
    ocl_est = OpenCLKraskovCMI(settings=settings)
    (mi_ocl, dist, n_range_var1, n_range_var2, n_range_cond) = ocl_est.estimate(
        source, target, source_uncorr
    )
...
def test_mi_correlated_gaussians_two_chunks():
    ...
    expected_mi, source, source_uncorr, target = _get_gauss_data(n=20000, seed=SEED)
    ...
    # Run JIDT estimator.
    jidt_est = JidtKraskovMI(settings={})
    mi_jidt = jidt_est.estimate(
        source[0 : int(n_points / 2), :], target[0 : int(n_points / 2), :]
    )

    print(
        "JIDT MI result: {0:.4f} nats; OpenCL MI result: [{1:.4f}, {2:.4f}] "
...
def test_cmi_uncorrelated_gaussians():
    ...
    settings = {"debug": True, "return_counts": True}
    ocl_est = OpenCLKraskovCMI(settings=settings)
    (mi_ocl, dist, n_range_var1, n_range_var2, n_range_var3) = ocl_est.estimate(
        var1, var2, var3
    )
    mi_ocl = mi_ocl[0]
...
def test_local_values():
    ...
    # Get data
    n_chunks = 2
    expec_mi, source, source_uncorr, target = _get_gauss_data(n=20000, seed=SEED)
    ...
    # Report results
    print(
        "OpenCL MI result: {0:.4f} nats (chunk 1); {1:.4f} nats (chunk 2) "
        "expected to be close to {2:.4f} nats for uncorrelated "
        "Gaussians.".format(mi_ch1, mi_ch2, expec_mi)
    )
    print(
        "OpenCL CMI result: {0:.4f} nats (chunk 1); {1:.4f} nats (chunk 2) "
        "expected to be close to {2:.4f} nats for uncorrelated "
...
def test_multi_gpu():
    ...
    mi_ocl = mi_ocl[0]
    print(
        "Expected MI: {0:.4f} nats; OpenCL MI result: {1:.4f} nats; "
        "expected to be close to 0 nats for uncorrelated "
