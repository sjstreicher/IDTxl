...
def _assert_result(results, expected_res, estimator, measure, tol=0.05):
    ...
    print(
        "{0} - {1} result: {2:.4f} nats expected to be close to {3:.4f} "
        "nats.".format(estimator, measure, results, expected_res)
    )
    assert np.isclose(
        results, expected_res, atol=tol
...
def _compare_result(res1, res2, estimator1, estimator2, measure, tol=0.05):
    ...
    print(
        "{0} vs. {1} - {2} result: {3:.4f} nats vs. {4:.4f} "
...
def _get_gauss_data(n=10000, covariance=0.4, expand=True, seed=None):
    ...
    target = [
        ...
        for pair in zip(
            ...
            [(1 - covariance) * y for y in [rn.normalvariate(0, 1) for r in range(n)]],
        )
    ]
...
def _get_ar_data(n=10000, expand=False, seed=None):
    ...
    for n in range(order, n + order):
        source1[n] = (
            term_1 * source1[n - 1] - 0.9025 * source1[n - 2] + np.random.normal()
        )
...
def test_mi_gauss_data():
    ...
    mi_uncor = mi_estimator.estimate(source2, target)
    _assert_result(
        mi_cor, expected_mi, "JidtDiscreteMI", "CMI (no cond.)", 0.08
    )  # More variability here
    _assert_result(
        mi_uncor, 0, "JidtDiscreteMI", "CMI (uncorr., no cond.)", 0.08
    )  # More variability here
...
def test_cmi_gauss_data_no_cond():
    ...
    mi_uncor = mi_estimator.estimate(source2, target)
    _assert_result(
        mi_cor, expected_mi, "JidtDiscreteCMI", "CMI (no cond.)", 0.08
    )  # More variability here
    _assert_result(
        mi_uncor, 0, "JidtDiscreteCMI", "CMI (uncorr., no cond.)", 0.08
    )  # More variability here
...
def test_cmi_gauss_data():
    ...
    mi_uncor = mi_estimator.estimate(source2, target)
    _assert_result(
        mi_cor, expected_mi, "JidtDiscreteCMI", "CMI (corr.)", 0.08
    )  # More variability here
    _assert_result(
        mi_uncor, 0, "JidtDiscreteCMI", "CMI (uncorr.)", 0.08
    )  # More variability here
...
def test_te_gauss_data():
    ...
    source2 = source2[1:]
    target = target[:-1]
    settings = {"discretise_method": "equal", "n_discrete_bins": 4, "history_target": 1}
    # Test Kraskov
    ...
    mi_uncor = mi_estimator.estimate(source2, target)
    _assert_result(
        mi_cor, expected_mi, "JidtDiscreteTE", "TE (corr.)", 0.08
    )  # More variability here
    _assert_result(
        mi_uncor, 0, "JidtDiscreteTE", "TE (uncorr.)", 0.08
    )  # More variability here
...
def test_ais_gauss_data():
    ...
    settings = {"discretise_method": "equal", "n_discrete_bins": 4, "history": 2}
    ...
    _compare_result(
        mi_cor_k, mi_cor_g, "JidtKraskovAIS", "JidtGaussianAIS", "AIS (AR process)"
    )
...
def test_one_two_dim_input_kraskov():
    ...
    expected_mi, src_one, s, target_one = _get_gauss_data(expand=False, seed=SEED)
    ...
    _compare_result(
        mi_cor_one, mi_cor_two, "JidtKraskovMI one dim", "JidtKraskovMI two dim", "MI"
    )
    # CMI
    ...
    _compare_result(
        mi_cor_one, mi_cor_two, "JidtKraskovMI one dim", "JidtKraskovMI two dim", "CMI"
    )
    # TE
    ...
    _compare_result(
        mi_cor_one, mi_cor_two, "JidtKraskovMI one dim", "JidtKraskovMI two dim", "TE"
    )
    # AIS
    ...
    _compare_result(
        mi_cor_one,
        mi_cor_two,
        "JidtKraskovAIS one dim",
        "JidtKraskovAIS two dim",
        "AIS (AR process)",
    )
...
def test_one_two_dim_input_gaussian():
    ...
    _compare_result(
        mi_cor_one, mi_cor_two, "JidtGaussianMI one dim", "JidtGaussianMI two dim", "MI"
    )
    # CMI
    ...
    _compare_result(
        mi_cor_one,
        mi_cor_two,
        "JidtGaussianMI one dim",
        "JidtGaussianMI two dim",
        "CMI",
    )
    # TE
    ...
    _compare_result(
        mi_cor_one, mi_cor_two, "JidtGaussianMI one dim", "JidtGaussianMI two dim", "TE"
    )
    # AIS
    ...
    _compare_result(
        mi_cor_one,
        mi_cor_two,
        "JidtGaussianAIS one dim",
        "JidtGaussianAIS two dim",
        "AIS (AR process)",
    )
...
def test_one_two_dim_input_discrete():
    ...
    mi_cor_one = mi_estimator.estimate(src_one, target_one)
    _assert_result(
        mi_cor_one, expected_mi, "JidtDiscreteMI", "MI", 0.08
    )  # More variability here
    mi_cor_two = mi_estimator.estimate(src_two, target_two)
    _assert_result(
        mi_cor_two, expected_mi, "JidtDiscreteMI", "MI", 0.08
    )  # More variability here
    _compare_result(
        mi_cor_one, mi_cor_two, "JidtDiscreteMI one dim", "JidtDiscreteMI two dim", "MI"
    )
    # CMI
    cmi_estimator = JidtDiscreteCMI(settings=settings)
    mi_cor_one = cmi_estimator.estimate(src_one, target_one)
    _assert_result(
        mi_cor_one, expected_mi, "JidtDiscreteCMI", "CMI", 0.08
    )  # More variability here
    mi_cor_two = cmi_estimator.estimate(src_two, target_two)
    _assert_result(
        mi_cor_two, expected_mi, "JidtDiscreteCMI", "CMI", 0.08
    )  # More variability here
    _compare_result(
        mi_cor_one,
        mi_cor_two,
        "JidtDiscreteMI one dim",
        "JidtDiscreteMI two dim",
        "CMI",
    )
    # TE
    ...
    mi_cor_one = te_estimator.estimate(src_one[1:], target_one[:-1])
    _assert_result(
        mi_cor_one, expected_mi, "JidtDiscreteTE", "TE", 0.08
    )  # More variability here
    mi_cor_two = te_estimator.estimate(src_one[1:], target_one[:-1])
    _assert_result(
        mi_cor_two, expected_mi, "JidtDiscreteTE", "TE", 0.08
    )  # More variability here
    _compare_result(
        mi_cor_one, mi_cor_two, "JidtDiscreteMI one dim", "JidtDiscreteMI two dim", "TE"
    )
    # AIS
    ...
    _compare_result(
        mi_cor_one,
        mi_cor_two,
        "JidtDiscreteAIS one dim",
        "JidtDiscreteAIS two dim",
        "AIS (AR process)",
    )
...
def test_local_values():
    ...
    _assert_result(
        np.mean(mi), expected_mi, "JidtDiscreteMI", "MI", 0.08
    )  # More variability here
    ...
    _assert_result(
        np.mean(mi), expected_mi, "JidtDiscreteCMI", "CMI", 0.08
    )  # More variability here
    assert type(mi) is np.ndarray, "Local values are not a numpy array."
    ...
    _assert_result(
        np.mean(mi), expected_mi, "JidtDiscreteTE", "TE", 0.08
    )  # More variability here
    assert type(mi) is np.ndarray, "Local values are not a numpy array."
...
def test_lagged_mi():
    ...
    _compare_result(
        mi_d, te_d, "JidtDiscreteMI", "JidtDiscreteTE", "lagged MI", tol=0.05
    )
    _compare_result(mi_k, te_k, "JidtKraskovMI", "JidtKraskovTE", "lagged MI", tol=0.05)
    _compare_result(
...
def test_jidt_kraskov_alg1And2():
    ...
    source = [
        sum(pair)
        for pair in zip(
            [y for y in range(n)], [rn.normalvariate(0, 0.000001) for r in range(n)]
        )
    ]
    source = np.array(source)
    ...
    for k in range(4, 16):
        ...
        _compare_result(
            mi_alg1, expected_alg1, "JidtDiscreteMI_alg1", "Analytic", "MI", tol=0.00001
        )
        ...
        _compare_result(
            mi_alg2, expected_alg2, "JidtDiscreteMI_alg2", "Analytic", "MI", tol=0.00001
        )
