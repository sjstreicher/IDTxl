...
def _verify_test(c_within, c_between, res):
    ...
    assert (
        c_between.adjacency_matrix_union[tp] == 1
    ).all(), "Missing union link in between network comparison."
    assert (
        c_within.adjacency_matrix_union[tp] == 1
    ).all(), "Missing union link in wihin network comparison."
    assert (
        c_between.adjacency_matrix_pvalue[tp] < 1
    ).all(), "Wrong p-value in between network comparison."
    assert (
        c_within.adjacency_matrix_pvalue[tp] < 1
    ).all(), "Wrong p-value in wihin network comparison."
    ...
    assert (
        c_between.adjacency_matrix_diff_abs[tp] > 0
    ).all(), "Missed difference in between network comparison."
    assert (
        c_within.adjacency_matrix_diff_abs[tp] > 0
    ).all(), "Missed difference in wihin network comparison."
