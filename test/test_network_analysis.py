...
def test_calculate_single_link():
    ...
    current_value_realisations = data.get_realisations(current_value, [current_value])[
        0
    ]
    ...
    for sources in ["all", [0]]:
        ...
        # conditioning set is built correctly for conditioning='full').
        cond_realisations = np.hstack(
            (  # cond. on second source and target
...
def test_lag_to_idx():
    ...
    assert np.array(results).all(), "Lags were not converted to indices correctly."
