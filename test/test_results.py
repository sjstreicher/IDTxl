...
def test_results_network_inference():
    ...
    for res in res_all:
        ...
        assert (
            adj_matrix.shape[0] == n_nodes
        ), "Incorrect number of rows in adjacency matrix."
        assert (
            adj_matrix.shape[1] == n_nodes
...
def test_results_network_comparison():
    ...
    for (i, res) in enumerate([res_within, res_between]):
        ...
        assert (
            res.get_adjacency_matrix("union")[s, t] == 1
        ).all(), "{0}-test did not return correct union network links.".format(test[i])
        no_diff = np.extract(
            np.invert(res.get_adjacency_matrix("comparison")),
            res.get_adjacency_matrix("union"),
        )
        assert (
            no_diff == 0
        ).all(), "{0}-test did not return 0 in union network for no links.".format(
            test[i]
        )
        # Comparison
        assert res.get_adjacency_matrix("comparison")[
            s, t
        ...
        assert (
            no_diff == 0
        ).all(), "{0}-test did not return 0 comparison for non-sign. links.".format(
            test[i]
        )
        # Abs. difference
        assert (
            res.get_adjacency_matrix("diff_abs")[s, t] > 0
        ...
        assert (
            no_diff == 0
        ).all(), "{0}-test did not return 0 difference for non-sign. links.".format(
            test[i]
        )
        # p-value
        p_max = 1 / comp_settings["n_perm_comp"]
        assert (
            res.get_adjacency_matrix("pvalue")[s, t] == p_max
        ...
        assert (
            no_diff == 1
        ).all(), "{0}-test did not return p-vals of 1 for non-sign. links.".format(
            test[i]
        )
