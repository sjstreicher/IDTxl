...
def test_data_type():
    ...
    assert issubclass(type(surr[0, 0]), np.float), "Realisations type is not a float."
    surr = stats._generate_spectral_surrogates(
        data=data, scale=1, n_perm=20, perm_settings=settings
    )
    assert issubclass(type(surr[0, 0, 0]), np.float), (
...
def test_analytical_surrogates():
    ...
    data = Data(np.vstack((source_dis, target_dis)), dim_order="ps", normalise=False)
    settings = {
