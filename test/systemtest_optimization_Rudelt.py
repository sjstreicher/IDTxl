...
def test_optimization_Rudelt_bbc():
    ...
    assert (
        round(res.opt_first_bin_size, 3) == 0.063
    ), "Estimated opt first bin size incorrect"
    assert round(res.firing_rate, 1) == 4.0, "Calculated firing rate incorrect"
    assert (
        round(res.recording_length, 1) == 952.2
    ), "Calculated recording length incorrect"
...
def test_optimization_Rudelt_bbc_noAveragedR_noAutoMI_noOutputImage():
    ...
    assert math.isclose(
        res.R_tot_CI[0], 0.107, rel_tol=1e-02
    ), "Estimated R_tot_CO_lo incorrect"
    assert math.isclose(
        res.R_tot_CI[1], 0.116, rel_tol=1e-02
    ), "Estimated R_tot_CO_hi incorrect"
    assert res.opt_number_of_bins_d == 5, "Estimated opt number of bins incorrect"
    assert round(res.opt_scaling_k, 2) == 0.44, "Estimated opt scaling k size incorrect"
    assert (
        round(res.opt_first_bin_size, 3) == 0.014
    ), "Estimated opt first bin size incorrect"
    assert round(res.firing_rate, 1) == 4.0, "Calculated firing rate incorrect"
    assert (
        round(res.recording_length, 1) == 952.2
...
def test_optimization_Rudelt_shuffling():
    ...
    assert math.isclose(
        res.T_D, 0.315, rel_tol=1e-02
    ), "Estimated T_D {0} incorrect".format(str(res.T_D))
    assert math.isclose(
        res.tau_R, 0.08, rel_tol=1e-01
    ), "Estimated tau_R {0} incorrect".format(str(res.tau_R))
    assert math.isclose(
        res.R_tot, 0.12, rel_tol=1e-02
    ), "Estimated R_tot {0} incorrect".format(str(res.R_tot))
    assert res.opt_number_of_bins_d == 5, "Estimated opt number of bins incorrect"
    assert round(res.opt_scaling_k, 1) == 0.0, "Estimated opt scaling k size incorrect"
    assert round(res.firing_rate, 1) == 4.0, "Calculated firing rate incorrect"
    assert (
...
def test_optimization_Rudelt_bbc_multiple_processes():
    ...
    for i in range(nr_processes):
        if i == 5:
            ...
            sampl = int(
                np.random.uniform(
                    low=0.5 * len(spiketimes), high=0.9 * len(spiketimes), size=(1,)
                )
            )
            nr_spikes[i] = sampl
    ...
    settings = {
        "embedding_past_range_set": [0.005, 0.31548, 0.62946, 2.50594, 5.0],
        "embedding_number_of_bins_set": [1, 3, 5],
        "number_of_bootstraps_R_max": 10,
        "number_of_bootstraps_R_tot": 10,
        "analyse_auto_MI": False,
...
def test_optimization_Rudelt_shuffling_multiple_processes():
    ...
    assert math.isclose(
        res2.R_tot, res5.R_tot, rel_tol=1e-02
    ), "R_tot of the same process differ. Needs to be identical"
    assert (
        res2.R_tot != res1.R_tot
    ), "R_tot of the different processes are Identical. Needs to be different"
    assert math.isclose(
        res2.T_D, res5.T_D, rel_tol=1e-02
    ), "T_D of the same process differ. Needs to be identical"
    assert (
        res2.T_D != res1.T_D
    ), "T_D of the different processes are Identical. Needs to be different"
    assert math.isclose(
        res2.tau_R, res5.tau_R, rel_tol=1e-02
    ), "tau_R of the same process differ. Needs to be identical"
